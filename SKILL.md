...
# 使用字幕样式预设烧录硬字幕
./scripts/generate_srt.py video.mp4 --burn --subtitlePreset default

# 使用图片风格的白底盒字幕，并把位置抬高一点
./scripts/generate_srt.py video.mp4 --burn --subtitlePreset shioneru

# 已有视频和 SRT 时直接烧录，仍可指定字幕样式预设
./scripts/generate_srt.py video.mp4 \
  --subtitle-file video.zh.srt \
  --burn \
  --subtitlePreset shioneru
...
| `--embed` | 封装软字幕到视频 |
| `--burn` | 烧录硬字幕到视频 |
| `--subtitlePreset` | 烧录字幕样式预设名（查找 `subtitlePreset/<name>.json`，仅对 `--burn` 生效） |
| `--subtitle-file` | 已有 SRT 字幕文件路径；与视频输入配合时可直接封装/烧录，跳过转录和翻译 |
...
- **字幕预设**: JSON 文件放在仓库根目录 `subtitlePreset/`，当前提供 `default.json` 和 `shioneru.json`
